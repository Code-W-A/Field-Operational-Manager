...
export default function Clienti() {
  ...
  const handleSearchChange = (value: string) => {
    console.log("👥🔍 CLIENTI search changed:", {
      newValue: value,
      oldValue: searchText,
    ...
    console.log("👥💾 Clienti search text saved to persistence")
  }
  ...
  useEffect(() => {
    console.log("👥🔄 CLIENTI filtering effect triggered:", {
      hasClienti: clienti && clienti.length > 0,
      clientiCount: clienti?.length || 0,
    ...
    if (!clienti || clienti.length === 0) {
      console.log("👥⚠️ No clienti data available for filtering")
      setFilteredData([])
      return
    }

    if (!searchText.trim() && !activeFilters.length) {
      console.log("👥➡️ No search or filters, using all clienti")
      setFilteredData(clienti)
      return
    }

    let filtered = clienti
    console.log("👥🎯 Starting clienti filtering process with", clienti.length, "clienti")

    // Apply active filters
    if (activeFilters.length) {
      console.log("👥🔧 Applying", activeFilters.length, "active filters to clienti")
      filtered = applyFilters(filtered)
      console.log("👥📊 After filters applied:", filtered.length, "clienti remain")
    }
    ...
    if (searchText.trim()) {
      ...
      filtered = filtered.filter((item) => {
        ...
        if (searchResult && searchText.toLowerCase().includes('conferin')) {
          console.log("👥🎯 CONFERINTA search match found in client:", {
            clientId: item.id,
            nume: item.nume,
            adresa: item.adresa,
    ...
    console.log("👥✅ Final clienti filtering result:", {
      originalCount: clienti.length,
