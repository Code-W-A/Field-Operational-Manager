...
import { getClienti, isContractAvailableForClient, validateContractAssignment, addLucrare, type Echipament, type Locatie } from "@/lib/firebase/firestore"
  ...
  const handleAddContract = async () => {
    ...
    try {
      ...
      if (
        ...
        const equipmentIds = (contractData.equipmentIds as string[])
        await addLucrare({
          client: client?.nume || "",
          persoanaContact: "",
          telefon: "",
          dataEmiterii: format(new Date(), "dd.MM.yyyy HH:mm"),
          dataInterventie: format(nextReviewDate, "dd.MM.yyyy HH:mm"),
          tipLucrare: 'Revizie',
          locatie: newContractLocationId || selectedLocation?.nume || "",
          descriere: `Revizie programată (contract ${contractData.number})`,
          ...
          equipmentIds,
          revision: {
            checklistVersionId: new Date().toISOString(),
            equipmentStatus: Object.fromEntries(equipmentIds.map((id: string) => [id, "pending"])),
            doneCount: 0,
          } as any,
          // extra scheduling metadata
          visibleAt: Timestamp.fromDate(generateDate) as any,
