          ...
          try {
            ...
            if (ww.autoGenerated && !ww.nextScheduled && ww.contractId && !!ww.echipament) {
              ...
              if (cSnap.exists()) {
                ...
                if (c?.recurrenceInterval && c?.recurrenceUnit) {
                  // Parse dataInterventie curentă
                  const currentReview = parse(String(ww.dataInterventie || ""), "dd.MM.yyyy HH:mm", new Date())
                  if (!isNaN(currentReview.getTime())) {
                    let nextReview = new Date(currentReview)
                    if (c.recurrenceUnit === 'luni') {
                      nextReview = addMonths(currentReview, c.recurrenceInterval)
                      if (c.recurrenceDayOfMonth && c.recurrenceDayOfMonth >= 1 && c.recurrenceDayOfMonth <= 31) {
                        const lastDay = new Date(nextReview.getFullYear(), nextReview.getMonth() + 1, 0).getDate()
                        nextReview.setDate(Math.min(c.recurrenceDayOfMonth, lastDay))
                      }
                    } else {
                      nextReview = addDays(currentReview, c.recurrenceInterval)
                    }
                    const daysBefore = c.daysBeforeWork || 10
                    const generateAt = addDays(nextReview, -daysBefore)

                    await addLucrare({
                      client: ww.client || "",
                      persoanaContact: ww.persoanaContact || "",
                      telefon: ww.telefon || "",
                      dataEmiterii: format(new Date(), "dd.MM.yyyy HH:mm"),
                      dataInterventie: format(nextReview, "dd.MM.yyyy HH:mm"),
                      tipLucrare: ww.tipLucrare || 'Revizie',
                      locatie: ww.locatie || "",
                      echipament: ww.echipament || "",
                      echipamentCod: ww.echipamentCod || "",
                      echipamentModel: ww.echipamentModel || '',
                      descriere: ww.descriere || `Revizie programată (contract ${ww.contractNumber || ww.contract || ''})`,
                      statusLucrare: 'Listată',
                      statusFacturare: 'Nefacturat',
                      tehnicieni: [],
                      contract: ww.contract || ww.contractNumber || '',
                      contractNumber: ww.contractNumber || ww.contract || '',
                      contractId: ww.contractId,
                      visibleAt: (Timestamp as any).fromDate(generateAt),
                      autoGenerated: true as any,
                    } as any)

                    // marcăm că am programat următoarea pentru a evita duplicatele
                    await updateLucrare(id, { nextScheduled: true }, undefined, undefined, true)
                  }
                }
              }
            }
