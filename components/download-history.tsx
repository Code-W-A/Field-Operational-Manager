...
export function DownloadHistory({ lucrareId }: { lucrareId: string }) {
  ...
  const [locationEmail, setLocationEmail] = useState<string | null>(null)
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        // Derivăm emailul persoanei de contact a locației pentru afișare când logul vine din portal ("portal")
        try {
          ...
          if (clientId) {
            ...
            if (loc) {
              ...
              if (isValid(exact?.email)) setLocationEmail(String(exact.email))
              else if (isValid(loc?.email)) setLocationEmail(String(loc.email))
              else {
                const anyContact = persoane.find((c: any) => isValid(c?.email))
                if (isValid(anyContact?.email)) setLocationEmail(String(anyContact.email))
              }
            }
          }
        } catch {}
    ...
    <div className="rounded border bg-white">
      <div className="divide-y">
        {items.map((it) => {
          ...
          const who = (it.userEmail && it.userEmail !== "portal") ? it.userEmail : (locationEmail || "Portal client")
          return (
