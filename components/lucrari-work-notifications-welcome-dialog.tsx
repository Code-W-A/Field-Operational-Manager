...
interface LucrariWorkNotificationsWelcomeDialogProps {
  isOpen: boolean
  onClose: () => void
  userName?: string
  lucrari: Lucrare[] // Primește datele ca prop în loc să facă query
}

export function LucrariWorkNotificationsWelcomeDialog({ 
  isOpen, 
  onClose,
  userName = "Utilizator",
  lucrari = [] // Folosește datele existente din pagina lucrări
}: LucrariWorkNotificationsWelcomeDialogProps) {
  const { summary, loading } = useLucrariWelcomeNotifications(lucrari)
