...
// Add the useFirestoreQuery function after the existing useFirebaseCollection function

export function useFirestoreQuery<T extends DocumentData>(queryFn: () => Query<DocumentData> | null, deps: any[] = []) {
  const [data, setData] = useState<T[]>([])
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState<Error | null>(null)
  const { isPreview, lucrari, clienti, users, logs } = useMockData()
  ...
  useEffect(() => {
    // If we're in preview mode, don't execute the query
    if (isPreview) {
      return () => {}
    }
    ...
    try {
      ...
      // If the query is null, return empty data
      if (!query) {
        setData([])
        setLoading(false)
        return () => {}
