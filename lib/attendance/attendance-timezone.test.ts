...
test("buildAttendanceEntriesFromSessions formats pontaj and extra logs in Bucharest time", () => {
  ...
  assert.equal((Object.values(entries[0]) as unknown[]).includes(undefined), false)
  assert.equal("selfieStartUrl" in entries[0], false)
  assert.equal("selfieEndUrl" in entries[0], false)
  assert.equal("lateStartMinutes" in entries[0], false)
